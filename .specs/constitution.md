...
* **一句話定義**：以任務需求為導向，透過數位孿生零成本自主演化有機飛行構型與 COTS 配置，並藉由全機 3D 列印製造、PX4/MAVLink 實機試飛進行 Real-to-Sim 參數自動反向校準，落實「人類主飛、AI 輔助介入」的虛實共生閉環系統。
* **產品願景**：徹底終結無人機硬體設計的經驗試錯與炸機成本，根據任務約束（載重、航程、狹小避障、抗風）自動演化長出最適幾何與電氣配置，在純瀏覽器 60 FPS WebGL 與原生 PX4/Gazebo 環境中完成端到端虛擬試飛驗證與自主飛行示範訓練數據收集；透過實機飛行自動校準參數，實現飛行器的自主適應進化。
...
* **關鍵使用場景 (Key Scenarios)**：
  ...
  5. **自主飛行示範與安全副駕駛 (AI Copilot)**：在虛擬世界中以自動化試錯探索極限碰撞邊界；在實機飛行中維持「人類主飛，AI 輔助介入」，在即將碰撞時自動啟動排斥力避障與速度箝位。
...
* **人類主飛，AI 輔助介入 (Shared Autonomy Copilot)**：自駕演算法定位為飛手的安全副駕駛，在手動操控時提供動態排斥力避障與速度箝位安全防線。
...
* [ ] **[M2.5] 「人類主飛，AI 輔助介入」協同飛控原型 (Shared Autonomy Copilot)**：在 WebGL 模擬器中實作飛手手動操縱與 AI 背景安全護欄（動態排斥力避障、失控介入、防撞速度箝位）。
