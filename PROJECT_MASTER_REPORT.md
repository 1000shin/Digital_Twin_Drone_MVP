...
| **v12.1.0** | 2026-09-25 | **[M2.5.2] 深度強化學習與特權導師-學生蒸餾極限避障系統 (Privileged Teacher-Student DRL)**：實作特權導師-學生蒸餾架構；環境擴充 32D 特權真值與 23D 帶噪聲機載感知；PyTorch 離線導師-學生蒸餾器 (`train_privileged_distillation.py`)；**方案 2 多階段學習時光軸快照**（`stage_0_untrained` 0% 初學、`stage_1_half_trained` 40% 中級、`stage_2_mastered` 100% 精通）；純 NumPy 學生推論引擎 (`StudentPolicyNumpy`，單步推論 0.0867ms，數值誤差 $\le 10^{-6}$）；WebGL 3D 模擬器原生 JS 神經推論與 HUD 學習時光軸檢視器 (`[🌱0%初學 | 🌿40%中級 | 🏆100%精通]`)；APF 韌性回退。 | `test_privileged_drl_distillation.py` (8/8 Pass)<br/>`python3 -m unittest` (64/64 Pass, 100%) |
| **v12.2.0** | 2026-09-25 | **[FEAT-M2.5.3] AI 自主飛行動力學三向優化 (自適應速度曲線、圓滑向心轉向、狹縫刀鋒特技穿障)**：直線空曠段提速至 $4.8\text{ m/s}$（提速 $>150\%$）；二階 S 曲線偏航與向心滾轉傾角補償（$\phi = \arctan(v \cdot \omega / g)$）；$58^\circ$ 刀鋒穿障（Knife-Edge Slit Traversal，水平投影寬度驟縮至 0.7m）；HUD 突發紅色障礙柱一鍵注入 (快捷鍵 O)；純 NumPy 向量化推論延遲驟降至 $< 0.02\text{ms}$。 | `test_webgl_environments.py` (69/69 Pass, 100%) |
| **v12.2.1** | 2026-09-25 | **[FIX-M2.5.3-HTML-MODEL-LOAD] WebGL 飛行測試模擬器 TDZ 作用域死鎖修復與畫面全黑問題根除**：提升 `dynamicTestObstacleObj` 宣告消除 `ReferenceError`；恢復 32 組件無人機與場景 3D 渲染與物理循環；自動化宣告次序回歸測試；全系統 README 與多媒體實體（MP4 錄影與 4 視角寫真）全面升級。 | `test_webgl_environments.py` (70/70 Pass, 100%) |
