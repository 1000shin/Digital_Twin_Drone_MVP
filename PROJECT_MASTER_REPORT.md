...
| **v12.1.0** | 2026-09-25 | **[M2.5.2] 深度強化學習與特權導師-學生蒸餾極限避障系統 (Privileged Teacher-Student DRL)**：實作特權導師-學生蒸餾架構；環境擴充 32D 特權真值與 23D 帶噪聲機載感知；PyTorch 離線導師-學生蒸餾器 (`train_privileged_distillation.py`)；**方案 2 多階段學習時光軸快照**（`stage_0_untrained` 0% 初學、`stage_1_half_trained` 40% 半熟、`stage_2_mastered` 100% 精通）；純 NumPy 學生推論引擎 (`StudentPolicyNumpy`，單步推論 0.0867ms，數值誤差 $\le 10^{-6}$）；WebGL 3D 模擬器原生 JS 神經推論與 HUD 學習時光軸檢視器 (`[🌱0%初學 | 🌿40%半熟 | 🏆100%精通]`)；APF 韌性回退。 | `test_privileged_drl_distillation.py` (8/8 Pass)<br/>`python3 -m unittest` (64/64 Pass, 100%) |
