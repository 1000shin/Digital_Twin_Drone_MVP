...
### Added
- **[FEAT-M2.5.3] AI 自主飛行動力學三向優化 (自適應速度曲線、圓滑向心轉向、狹縫刀鋒特技穿障)**:
  - **自適應速度曲線 (Adaptive Velocity Profiling)**: 解決 AI 巡弋速度過慢問題。空曠直線航段巡航目標速度由固定 $1.6\text{ m/s}$ 大幅提升至 $4.8\text{ m/s}$（提速 $> 150\%$），前傾角限制放寬至 $0.48\text{ rad}$（$27.5^\circ$）釋放強勁水平推力；門前進門對齊平穩降速至 $2.4\text{ m/s}$，出門走廊提速至 $3.6\text{ m/s}$，轉角過渡維持 $2.8 \sim 4.2\text{ m/s}$。
  - **前瞻軌跡引導與空氣動力向心轉向 (Smooth Pure Pursuit & Coordinated Banking Turn)**: 解決轉彎生硬直角折線與甩尾問題。出門走廊與外環轉角導入前瞻切線引導點預插值，消除折線頂點硬切；偏航角導入二階 S 曲線加速度限制（$0.012\text{ rad/tick}^2$）根除機頭瞬間抽搐；融入空氣動力向心滾轉傾角補償（$\phi = \arctan(v \cdot \omega / g)$），過彎時機身自動向內側傾斜 $10^\circ \sim 20^\circ$，利用升力水平分量提供向心力，呈現極具航模與真機美感的圓滑側傾走線。
  - **狹道可通行性預判與高敏捷刀鋒彈射加速穿障 (Knife-Edge Slit Traversal)**: 徹底刪除過小狹縫消極繞道目標。機載 LiDAR 提早 2.5m 預判前方障礙狹縫淨寬與高度，當間距小於水平翼展（$0.65\text{m} \le W_{\text{gap}} < 1.6\text{m}$）時，求解最佳側傾穿障角，進縫前迅速將機身翻滾至 $58^\circ$（水平投影寬度驟縮至 $0.7\text{m}$），同時前向推力大爆發加速至 $4.8\text{ m/s}$「彈射穿透」狹縫以克服大傾角掉高，出縫瞬間極速反向回正平飛；常規門框強制姿態平整（$|\text{Roll}| \le 8^\circ$）與雙邊平衡居中。
  - **HUD 原生避障實測按鈕 (1-Click Hazard Injection)**: 3D 模擬器 HUD 面板內建「🚨 插入突發紅色柱子 (避障實測) (O)」，支援鍵盤快捷鍵 `O`，可一鍵在航道中央動態插入/移除發光測試立柱，直觀檢視 AI 即時避障與刀鋒特技。
  - **純 NumPy 學生推論器向量化加速**: 在 `autonomous_flight_learner.py` 中為 `StudentPolicyNumpy` 導入預快取矩陣向量化運算，單步推論延遲由 $0.15\text{ms}$ 驟降至 $< 0.02\text{ms}$。
  - **全套單元測試擴充**: 新增 3 項針對 FEAT-M2.5.3 的自動化單元測試，全專案 69/69 項測試 100% 通過（耗時 10.51s）。
